# -*- coding: utf-8 -*-
"""
Created on Sun Nov 16 16:01:11 2025
...
# ================================================================
def find_resonance_peaks(freqs, H_dB, height_threshold=3, distance=None):
    """
    Finds resonance peaks in |H(f)|.

    Parameters:
        height_threshold  – minimum prominence in dB
        distance          – minimum index spacing between peaks

    Returns:
        peak_freqs, peak_values
    """
    peaks, _ = find_peaks(H_dB, prominence=height_threshold, distance=distance)
    return freqs[peaks], H_dB[peaks]

"""
def resonance_spacing(peak_freqs):
    
    Computes spacing between resonances: Δf_n = f_(n+1) - f_n
    
    if len(peak_freqs) < 2:
        return np.array([])
    return np.diff(peak_freqs)
"""
#plots
def plot_transfer_function(freqs, H_dB, peaks=None):
    plt.figure(figsize=(10, 5))
    plt.semilogx(freqs, H_dB, '-k', label="Magnitude (dB)")
    if peaks is not None:
        plt.semilogx(peaks[0], peaks[1], 'or', label="Resonances")
    plt.xlabel("Frequency (Hz)")
...
if __name__ == "__main__":
    ...
    if mode == "s":
        ...
        measure_sweep_raw(
        myDAQ, analyzer, freqs, amplitude=3.0, repeats=3
        )
    
    
    elif mode == "a":
        #from 10 hz to 200kHz
        freqs = np.logspace(1, 5.3, 100)
        ...
        measure_sweep_raw(
            ...
            amplitude=3.0,
            repeats=3,
            filename="analog_data.npz")
        
        #read out data from saved file
        freqs, Uin_all, Uout_all, t = myDAQ.load_data("analog_data.npz")
        ...
        height_threshold = 3
        print("Find resonance peaks.")
        peak_freqs, peak_vals = find_resonance_peaks(
        freqs, H_dB, height_threshold)

        print("Computing resonance spacing.")
        ...
        print(results["peak_freqs"])

        print("\nResonance spacing (Hz):")
        print(results["spacing"])

    # Plot result
    plot_transfer_function(results["freqs"], results["H_dB"],
                           (results["peak_freqs"], results["peak_vals"]))
